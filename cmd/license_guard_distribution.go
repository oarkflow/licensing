...
func runWithDistributionLicense(ctx context.Context, run func(context.Context)) {
	...
	if err != nil {
		log.Fatalf("Distribution license configuration failed: %v", err)
	}
	...
	if err := validateDistributionLicense(cfg, licenseData.ProductID, licenseData.IsRevoked, licenseData.ExpiresAt, licenseData.HasFeature); err != nil {
