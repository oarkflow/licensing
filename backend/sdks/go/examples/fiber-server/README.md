...
2. **Create Client & License**: Use the admin API to create a client and license:
   ...
   # Create a client
   curl -X POST http://localhost:6601/api/clients \
   ...
   # Create a license
   curl -X POST http://localhost:6601/api/licenses \
     -H "Content-Type: application/json" \
     -H "X-API-Key: $API_KEY" \
     -d '{
...
|------|---------|-------------|
| `--server` | `http://localhost:6601` | License server URL |
...
# Create product
curl -X POST http://localhost:6601/api/products \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "SecretR",
...
# Create features
curl -X POST http://localhost:6601/api/products/PRODUCT_ID/features \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "GUI",
...
curl -X POST http://localhost:6601/api/products/PRODUCT_ID/features/FEATURE_ID/scopes \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "List",
