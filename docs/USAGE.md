...
2. **Start the server (development mode allows HTTP):**
   ...
   go run ./cmd/server --http-addr :6601 --allow-insecure-http
   ```
...
3. **Create a client record:**
   ```bash
   curl -sSL -X POST http://localhost:6601/api/clients \
...
4. **Issue a license with a plan + check policy:**
   ```bash
   curl -sSL -X POST http://localhost:6601/api/licenses \
     -H 'Content-Type: application/json' \
     -H 'X-API-Key: dev-admin-key' \
     -d '{
       "client_id": "<CLIENT_ID>",
...
6. **Run the client and wrap an example app:**
   ...
   go run ./client \
     ...
     --server-url http://localhost:6601 \
