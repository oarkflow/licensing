...
- **Base URL** – default development server runs at `http://localhost:6601`; production deployments should expose `https://<your-domain>`.
...
```bash
curl -sSL -X POST "http://localhost:6601/api/admin/licenses/provision" \
...
export BASE_URL="${LICENSE_SERVER_URL:-http://localhost:6601}"
