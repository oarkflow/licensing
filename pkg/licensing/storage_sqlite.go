...
func NewSQLiteStorage(path string) (*SQLiteStorage, error) {
	...
	if err := ensureCRMSchema(db); err != nil {
		return nil, err
	}
	crmRepo, err := crm.NewSQLRepository(db, "sqlite")
	if err != nil {
		return nil, fmt.Errorf("failed to initialize crm repository: %w", err)
	}
	return &SQLiteStorage{db: db, crmRepo: crmRepo}, nil
}

// CRMRepository exposes the durable CRM repository backed by the shared SQLite database.
func (s *SQLiteStorage) CRMRepository() crm.Repository {
	return s.crmRepo
}
func ensureCRMSchema(db *squealx.DB) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS tenant_settings (
			...
			login_modes TEXT NOT NULL DEFAULT '[]',
	...
	for _, stmt := range stmts {
		if _, err := db.Exec(stmt); err != nil {
			return fmt.Errorf("sqlite crm schema: %w", err)
		}
	}
	return nil
}
...
func ensureSQLiteSchema(db *squealx.DB) error {
	...
	if err := ensureSQLiteColumn(db, "licenses", "product_id", "TEXT"); err != nil {
		return err
	}
	if err := ensureSQLiteColumn(db, "licenses", "plan_id", "TEXT"); err != nil {
		return err
	}
	if err := ensureSQLiteColumn(db, "licenses", "entitlements", "TEXT"); err != nil {
		return err
	}
...
func scanLicenseRow(scanner rowScanner) (*License, error) {
	var lic License
	var productID, planID sql.NullString
	...
	if productID.Valid {
		lic.ProductID = productID.String
	}
	if planID.Valid {
		lic.PlanID = planID.String
	}
	if revokedAt.Valid {
	...
	if entitlementsJSON.Valid && entitlementsJSON.String != "" {
...
func (s *SQLiteStorage) SaveLicense(ctx context.Context, license *License) error {
	...
	return s.withTx(ctx, func(tx squealx.SQLTx) error {
		...
		if license.Entitlements != nil {
			data, err := json.Marshal(license.Entitlements)
			if err != nil {
				return fmt.Errorf("failed to marshal entitlements: %w", err)
			}
			entitlementsJSON = sql.NullString{String: string(data), Valid: true}
		}
		...
		_, err := tx.ExecContext(ctx, query,
			...
			license.ClientID,
			license.Email,
			nullString(license.ProductID),
			nullString(license.PlanID),
			license.PlanSlug,
			license.LicenseKey,
